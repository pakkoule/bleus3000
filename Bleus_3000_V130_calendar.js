/* 3615 Bleus V1.1.41 — Calendrier compact + chaînes + pays normalisés */
(()=>{
  'use strict';
  const state={matches:[],mode:'upcoming',team:'all',canEdit:false,live:new Map(),liveTimer:null,selections:[],competitions:[],homePage:1,homePageSize:4,featureStyle:null};
  ...
  function renderHomePagination(total){const host=$('#calendarHomePagination');if(!host)return;const pages=Math.max(1,Math.ceil(total/state.homePageSize));state.homePage=Math.min(Math.max(1,state.homePage),pages);if(pages<=1){host.innerHTML='';host.hidden=true;return;}host.hidden=false;const nums=[];for(const n of [1,2,state.homePage-1,state.homePage,state.homePage+1,pages-1,pages])if(n>=1&&n<=pages&&!nums.includes(n))nums.push(n);nums.sort((a,b)=>a-b);let prev=0,html='';for(const n of nums){if(prev&&n-prev>1)html+='<span>…</span>';html+=`<button type="button" data-home-page="${n}" class="${n===state.homePage?'is-active':''}">${n}</button>`;prev=n;}host.innerHTML=`<button type="button" data-home-page="${Math.max(1,state.homePage-1)}" ${state.homePage===1?'disabled':''}>‹</button>${html}<button type="button" data-home-page="${Math.min(pages,state.homePage+1)}" ${state.homePage===pages?'disabled':''}>›</button>`;host.querySelectorAll('[data-home-page]').forEach(b=>b.addEventListener('click',()=>{if(b.disabled)return;state.homePage=Number(b.dataset.homePage)||1;render();document.querySelector('.calendar-home')?.scrollIntoView({behavior:'smooth',block:'start'});}));}
  function render(){const all=filtered();const home=$('#calendarHomeList');if(home){const upcoming=state.matches.filter(m=>isLive(m)||(parseDate(eff(m,'match_date'))?.getTime()||0)>=Date.now()).sort((a,b)=>{if(isLive(a)!==isLive(b))return isLive(a)?-1:1;return parseDate(eff(a,'match_date'))-parseDate(eff(b,'match_date'));});const pages=Math.max(1,Math.ceil(upcoming.length/state.homePageSize));if(state.homePage>pages)state.homePage=pages;const start=(state.homePage-1)*state.homePageSize,up=upcoming.slice(start,start+state.homePageSize);home.innerHTML=up.map(row).join('')||'<div class="calendar-empty">Aucun prochain match enregistré.</div>';renderHomePagination(upcoming.length);}const full=$('#calendarFullList');if(full)full.innerHTML=all.map(row).join('')||'<div class="calendar-empty">Aucun match pour ce filtre.</div>';const add=$('#calendarAddBtn');if(add)add.hidden=!state.canEdit;}
  ...
  async function openCreate(){
    ...
    $('#calendarCreateHomeAway').value='home';$('#calendarCreateFranceScore').value='';$('#calendarCreateOpponentScore').value='';if($('#calendarCreateFeatureMode'))$('#calendarCreateFeatureMode').value='auto';
