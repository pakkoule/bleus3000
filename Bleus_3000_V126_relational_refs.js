/* 3615 Bleus V1.1.41 — Référentiels relationnels : pays normalisés + diffusions */
(() => {
  'use strict';
  ...
  async function load(){
    if(loaded)return matches;if(loading)return loading;loading=(async()=>{
      ...
      const [oo,ccRes,ll,hh]=await Promise.all([
        fetchIn('opponents','id,name,fifa_code',oppIds),client.from('competitions').select('id,name,edition,tag_id,gender,selection_category,competition_type,status').order('name').limit(5000),fetchIn('places','id,name,city,country',placeIds),fetchIn('personnel','id,display_name',coachIds)
      ]);
      if(ccRes.error)throw ccRes.error;competitionRows=ccRes.data||[];const om=new Map(oo.map(x=>[x.id,x])),cm=new Map(competitionRows.map(x=>[x.id,x])),lm=new Map(ll.map(x=>[x.id,x])),hm=new Map(hh.map(x=>[x.id,x]));
      ...
      people=pp||[];
      const d=window.BLEUS3000_DATA;if(d?.references){const mr=d.references.find(x=>x.key==='matchs'),cr=d.references.find(x=>x.key==='competitions'),pr=d.references.find(x=>x.key==='personnel');if(mr)mr.count=String(matches.length);if(cr)cr.count=String(competitionRows.length);if(pr)pr.count=String(people.length);}
  ...
  function renderMatchCard(m){
    const opp=countryName(effective(m,'opponent_name')||'Adversaire');const home=m.home_away==='home';const sel='France';const left=home?sel:opp,right=home?opp:sel;const a=effective(m,'france_score'),b=effective(m,'opponent_score');const has=a!==null&&a!==undefined&&a!==''&&b!==null&&b!==undefined&&b!==''&&Number.isFinite(Number(a))&&Number.isFinite(Number(b));const future=(parseDate(effective(m,'match_date'))?.getTime()||0)>=Date.now();const score=has?(home?`${a} – ${b}`:`${b} – ${a}`):(future?'VS':'—');const place=[effective(m,'venue_name'),effective(m,'city')].filter(Boolean).join(' · ');const comp=effective(m,'competition_name')||sel;
    return `<article class="rel-ref-tile rel-match-tile" data-match-id="${esc(m.id)}"><div class="rel-ref-head"><div><small>${esc(fmtDateLong(effective(m,'match_date')))}</small><h3><span class="rel-team-name">${flagImg(left)}${esc(left)}</span> <span class="rel-score ${scoreClass(m)}">${esc(score)}</span> <span class="rel-team-name">${esc(right)}${flagImg(right)}</span></h3><div class="subtitle">${esc(comp)}${m.phase?` · ${esc(m.phase)}`:''}</div></div><span class="rel-result ${scoreClass(m)}">${resultLetter(m)}</span></div><div class="rel-facts">${place?`<span>🏟 ${esc(place)}</span>`:''}${m.coach?.display_name?`<span>👔 ${esc(m.coach.display_name)}</span>`:''}${m.spectators?`<span>👥 ${Number(m.spectators).toLocaleString('fr-FR')}</span>`:''}${effective(m,'broadcast_text')?(window.BLEUS3000_BROADCASTS?.renderText?.(effective(m,'broadcast_text'))||`<span>📺 ${esc(effective(m,'broadcast_text'))}</span>`):''}<span>${esc(m.lineup_status||m.notes_short||m.status||'Match')}</span></div><div class="rel-match-bottom"><div class="rel-tags">${chipForMatch(m)}${competitionChipForMatch(m)}</div><button class="rel-edit-match" type="button" data-calendar-edit="${esc(m.id)}">✎ Modifier</button></div></article>`;
  }
  function renderCompetitionCard(c){const tag=c.tag_id?tagsById.get(c.tag_id):null;const matchCount=matches.filter(m=>String(m.competition_id)===String(c.id)).length;return `<article class="rel-ref-tile rel-competition-tile"><div class="rel-ref-head"><div><h3>${esc(c.name)}</h3><div class="subtitle">${esc([c.edition,c.selection_category,c.gender==='F'?'Féminin':c.gender==='M'?'Masculin':''].filter(Boolean).join(' · ')||'Compétition')}</div></div><span class="rel-result">${matchCount}</span></div><div class="rel-facts"><span>⚽ ${matchCount} match${matchCount>1?'s':''} relié${matchCount>1?'s':''}</span>${c.status?`<span>État : ${esc(c.status)}</span>`:''}</div><div class="rel-tags">${tag?tagChip(tag,'relational-competition-chip'):''}</div></article>`;}
  ...
  async function render(kind,q=''){
    ...
    try{await load();if(kind==='matchs'){renderMatchFilters();let list=matches.filter(m=>matchQuery(q,[selectionLabel(m),effective(m,'opponent_name'),effective(m,'competition_name'),m.phase,effective(m,'venue_name'),effective(m,'city'),m.coach?.display_name,fmtDate(effective(m,'match_date')),m.provider]));list=applyMatchFilters(list);if(title)title.textContent='Matchs';if(sub)sub.textContent='Toutes les sélections françaises · historique et calendrier · données modifiables';if(count)count.textContent=`${list.length} match${list.length>1?'s':''}`;host.innerHTML=list.length?`<div class="rel-ref-grid rel-match-grid">${list.map(renderMatchCard).join('')}</div>`:'<div class="universal-search-empty">Aucun match ne correspond aux filtres.</div>';
      }else if(kind==='competitions'){hideMatchFilters();const list=competitionRows.filter(c=>matchQuery(q,[c.name,c.edition,c.selection_category,c.competition_type,c.status,tagsById.get(c.tag_id)?.label_text]));if(title)title.textContent='Compétitions';if(sub)sub.textContent='Entrées de compétition relationnelles · tags de familles partagés entre les éditions';if(count)count.textContent=`${list.length} compétition${list.length>1?'s':''}`;host.innerHTML=list.length?`<div class="rel-ref-grid">${list.map(renderCompetitionCard).join('')}</div>`:'<div class="universal-search-empty">Aucune compétition ne correspond à la recherche.</div>';
  ...
  function search(q){if(!loaded||!String(q||'').trim())return [];return searchRegistry.filter(x=>matchQuery(q,[x.title,x.meta,...x.values])).slice(0,20).map(x=>({type:x.kind==='matchs'?'Matchs':x.kind==='competitions'?'Compétitions':'Personnel & Officiels',icon:x.kind==='matchs'?'⚽':x.kind==='competitions'?'🏆':'👔',title:x.title,meta:x.meta,score:0.18,action:()=>{window.BLEUS3000_APP?.openReferences?.(x.kind);setTimeout(()=>{const i=$('#referenceSearch');if(i){i.value=x.title;i.dispatchEvent(new Event('input',{bubbles:true}));}},80);}}));}
  ...
  window.BLEUS3000_RELATIONAL_REFS={render,load,search,getMatch,effective,baseValue,applyLocalOverride,invalidate,getTag:id=>tagsById.get(id)||null,getTags:()=>[...tagsById.values()],getSelections:()=>selectionRows.map(x=>({...x})),getCompetitions:()=>competitionRows.map(x=>({...x})),getSectionTags:()=>{const ids=new Set(selectionRows.map(x=>x.team_tag_id).filter(Boolean));return [...tagsById.values()].filter(t=>ids.has(t.id)||t.reference_scope==='selection');},getCompetitionTags:()=>{const ids=new Set(competitionRows.map(x=>x.tag_id).filter(Boolean));return [...tagsById.values()].filter(t=>t.reference_scope==='competition'||ids.has(t.id)||String(t.slug||'').startsWith('competition-')||String(t.slug||'')==='match-amical');},get matches(){return matches;},get people(){return people;}};
  load().catch(()=>{});
})();
