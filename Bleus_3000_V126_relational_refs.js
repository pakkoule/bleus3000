/* 3615 Bleus V1.1.61.16 — Référentiels relationnels · filtres enrichis */
(() => {
  'use strict';
  ...
  function providerUnmatchedRow(r){const num=Number.isInteger(Number(r?.number))?`<b>${Number(r.number)}</b>`:'<b>—</b>';return `<div class="match-sheet-player is-provider-unmatched">${num}<span><strong>${esc(r?.name||'Joueur non relié')}</strong><small>${esc(r?.position||'Donnée fournisseur')}</small></span><em title="Nom reçu du fournisseur mais non relié avec certitude à une fiche joueur">API</em></div>`;}
  function renderMatchSheet(sheet,m){
    const rows=sheet.appearances||[],starters=rows.filter(r=>r.starter===true||norm(r.squad_status)==='starter'||norm(r.squad_status)==='titulaire'),others=rows.filter(r=>!starters.includes(r)),unmatched=providerUnmatchedPlayers(m);
  ...
  async function saveMatchSheetEditor(){
    ...
    try{const appearances=collectEditorAppearances(modal,state);const goals=collectEditorGoals(modal,state),cards=collectEditorCards(modal,state);await saveEditorAppearances(state,appearances);const jerseySelect=$('#matchSheetEditorJersey',modal);if(jerseySelect&&!jerseySelect.disabled&&window.BLEUS3000_JERSEYS?.setMatchJersey)await window.BLEUS3000_JERSEYS.setMatchJersey(state.matchId,jerseySelect.value||'',state.match?.selection_team_id||state.match?.selection?.id||null);if(state.eventsDirty){await saveEditorGoals(state,goals);await saveEditorCards(state,cards);await syncAppearanceEventCounters(state.matchId,goals,cards);}const manualStatus=appearances.length?`Feuille éditoriale · ${appearances.length} joueur${appearances.length>1?'s':''}`:null;const {error:matchStatusError}=await client.from('matches').update({lineup_status:manualStatus,updated_at:new Date().toISOString()}).eq('id',state.matchId);if(matchStatusError)throw matchStatusError;if(state.match)state.match.lineup_status=manualStatus;window.dispatchEvent(new CustomEvent('bleus:match-sheet-updated',{detail:{matchId:state.matchId,lineupStatus:manualStatus,source:'manual'}}));matchSheetCache.delete(state.matchId);if(st){st.className='match-sheet-editor-status is-ok';st.textContent='Feuille de match enregistrée ✓';}await refreshMatchSheetPanel(state.matchId);setTimeout(()=>{modal.hidden=true;matchSheetEditorState=null;},650);}catch(err){console.error('Édition feuille de match',err);if(st){st.hidden=false;st.className='match-sheet-editor-status is-error';st.textContent=String(err?.message||err);}}
  }
