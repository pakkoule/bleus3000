...
(() => {
  ...
  function render(){const h=ensureHost();if(!h)return;const state=window.C3K_ACCOUNT_STATE||{},vals=selected();if(!state.profile||!vals.length){h.hidden=true;h.innerHTML='';return;}h.hidden=false;h.innerHTML=`<div class="c3k-profile-shortcuts-grid">${vals.map(k=>{const o=options.find(x=>x.key===k);return `<button type="button" class="c3k-profile-shortcut" data-shortcut="${o.key}" title="${o.label}" aria-label="${o.label}"><span class="c3k-profile-shortcut-icon">${o.icon}</span><span class="c3k-profile-shortcut-tooltip" role="tooltip">${o.label}</span></button>`;}).join('')}</div>`;h.querySelectorAll('[data-shortcut]').forEach(b=>b.addEventListener('click',()=>{const o=options.find(x=>x.key===b.dataset.shortcut),t=o&&document.querySelector(o.selector);t?.click();}));}
