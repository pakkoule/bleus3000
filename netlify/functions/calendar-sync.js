...
const jheaders=(secret,extra={})=>({'apikey':secret,'Authorization':`Bearer ${secret}`,'Content-Type':'application/json',...extra});
const enc=v=>encodeURIComponent(v);
exports.handler=async()=>{
  const footballKey=process.env.API_FOOTBALL_KEY||process.env.APISPORTS_KEY;
  const supabaseUrl=process.env.SUPABASE_URL;
  const supabaseSecret=process.env.SUPABASE_SECRET_KEY||process.env.SUPABASE_SERVICE_ROLE_KEY;
  let teamMap={}; try{teamMap=JSON.parse(process.env.BLEUS_API_TEAM_MAP||'{}')}catch{}
  if(!footballKey||!supabaseUrl||!supabaseSecret||!Object.keys(teamMap).length){console.log('calendar-sync: configuration incomplète');return {statusCode:200,body:'configuration incomplete'};}
  ...
  for(const [selectionCode,teamIdRaw] of Object.entries(teamMap)){
    try{
      const teamId=Number(teamIdRaw); if(!teamId)continue;
      const selection=await first('selection_teams',`code=eq.${enc(selectionCode)}&select=id,code,name,gender,category`);if(!selection){console.log('Sélection inconnue',selectionCode);continue;}
    ...
    }catch(e){errors++;console.error('calendar-sync',selectionCode,e.message)}
  }
