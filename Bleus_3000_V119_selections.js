/* 3615 Bleus V1.1.39 — Sélections multi-contextes · tags auto → stats + GENERAL */
(() => {
  'use strict';
  ...
  let currentEditor=null,pendingPhoto=null,pendingPhotoPreview='',pendingJerseyNumbers=[],pendingAchievements=[];
  ...
  function achievementBadgeHtml(item){const a=item?.achievement||item||{},v=item?.achievement_value;const src=achievementIconSrc(a);return `<span class="selection-achievement-badge" title="${esc(a.label_text||'Accomplissement')}${v!==null&&v!==undefined?' · ×'+esc(v):''}">${src?`<img src="${esc(src)}" alt="${esc(a.label_text||'Accomplissement')}">`:`<span class="selection-achievement-fallback">${esc(a.icon_text||'🏅')}</span>`}${v!==null&&v!==undefined&&v!==''?`<span class="selection-achievement-count">×${esc(v)}</span>`:''}</span>`;}
  ...
  async function loadTeams(){
    ...
    if(te)throw new Error('Sélections · '+te.message);if(be)throw be;if(tge)throw tge;if(trle)throw trle;if(tce)throw tce;if(ache)throw ache;if(asce)throw asce;teams=t||[];teamCounts=new Map((tc||[]).map(x=>[x.selection_id,Number(x.player_count||0)]));borders=new Map((b||[]).map(x=>[x.selection_id,x]));tags=tg||[];tagReferenceLinks=trl||[];achievementScopes=asc||[];const allowed=new Set(achievementScopes.filter(x=>x.reference_type==='selection').map(x=>x.achievement_id));achievements=(ach||[]).filter(a=>allowed.has(a.id));
  }
  ...
  const POSITION_CHOICES=['Gardien','Défenseur central','Latéral droit','Latéral gauche','Piston droit','Piston gauche','Milieu défensif','Milieu central','Milieu offensif','Ailier droit','Ailier gauche','Avant-centre'];
  function playerPositions(p){return [...new Set([p?.primary_position,...(p?.secondary_positions||[])].map(x=>String(x||'').trim()).filter(Boolean))];}
  function positionsLabel(p){const a=playerPositions(p);return a.length?a.join(' · '):'non renseigné';}
  function positionsEditorHtml(p={}){const current=playerPositions(p),selected=new Set(current),choices=[...new Set([...current,...POSITION_CHOICES])];return `<div class="selection-position-checks">${choices.map(pos=>`<label class="selection-position-check"><input type="checkbox" name="positions" value="${esc(pos)}" ${selected.has(pos)?'checked':''}><span>${esc(pos)}</span></label>`).join('')}</div><small>Le premier poste sélectionné sera utilisé comme poste principal ; les autres seront conservés comme postes secondaires.</small>`;}
  ...
  function generalStats(contexts){return {selection_code:'GENERAL',selection_name:'GENERAL',selections:sumKnown(contexts,'selections'),goals:sumKnown(contexts,'goals'),starts:sumKnown(contexts,'starts'),wins:sumKnown(contexts,'wins'),draws:sumKnown(contexts,'draws'),losses:sumKnown(contexts,'losses'),international_number:null,appearance_status:'aggregate'};}
  function staticStatBox(label,value){return `<div class="sel-stat"><small>${esc(label)}</small><strong>${statVal(value)}</strong></div>`;}
  function contextStatsGrid(stats,isLive=false,row=null){return `${isLive&&row?statBox('Sélections','selections',stats.selections,row):staticStatBox('Sélections',stats.selections)}${isLive&&row?statBox('Buts','goals',stats.goals,row):staticStatBox('Buts',stats.goals)}${isLive&&row?statBox('Titularisations','starts',stats.starts,row):staticStatBox('Titularisations',stats.starts)}${isLive&&row?statBox('Victoires','wins',stats.wins,row):staticStatBox('Victoires',stats.wins)}${isLive&&row?statBox('Nuls','draws',stats.draws,row):staticStatBox('Nuls',stats.draws)}${isLive&&row?statBox('Défaites','losses',stats.losses,row):staticStatBox('Défaites',stats.losses)}`;}
  ...
  function drawRows(visible,total){
    ...
    host.innerHTML=`<div class="selection-grid">${visible.map(r=>{const p=publicPlayer(r);return `<article class="selection-player-tile" data-player-tile="${esc(r.player_id)}">
      <div class="selection-player-head"><div class="selection-photo-frame" style="${borderStyle()}"><div class="selection-photo-inner">${playerPhoto(p)}</div></div><div class="selection-player-title">${appearanceBadge(r)}<h3>${esc(p.display_name||'Joueur')}</h3><div class="selection-dob">Né(e) le ${esc(fmtDate(p.birth_date))}</div><div class="selection-position ${playerPositions(p).length?'':'is-empty'}">Poste${playerPositions(p).length>1?'s':''} · ${esc(positionsLabel(p))}</div><div class="selection-tile-actions">${canEdit()?`<button type="button" data-edit-player="${r.player_id}" title="Modifier">✎</button>`:''}</div></div></div>
      <div class="selection-stats-grid">${contextStatsGrid(r,true,r)}</div>
      <div class="selection-linked-fields"><div class="selection-jersey-field"><small>Numéros portés</small><div data-jersey-player="${r.player_id}" class="selection-jersey-slot"><span class="selection-jersey-empty">—</span></div></div><div class="selection-achievements-field"><small>Accomplissements</small><div data-ach-player="${r.player_id}" class="selection-achievements-slot"><span class="selection-achievement-empty">—</span></div></div></div>
      <div class="selection-tags" data-tags-player="${r.player_id}">${selectionTagHtml(team,r)}</div>
      <footer><div class="contributors" data-contrib-player="${r.player_id}">${contributorHtml(r._contributors)}</div></footer>
  ...
  async function hydrateLinkedFields(visible){
    ...
    const [{data:pa},{data:et},{data:allStats},{data:allJerseys}]=await Promise.all([
      ...
      client.from('player_selection_stats').select('player_id,selection_id,selections,goals,starts,wins,draws,losses,international_number,appearance_status,selection:selection_teams(id,code,name)').in('player_id',ids),
      client.from('player_jersey_numbers').select('player_id,selection_id,shirt_number').in('player_id',ids)
    ]);
    ...
    ids.forEach(id=>{
      ...
      const aa=(pa||[]).filter(x=>x.player_id===id);const ae=$(`[data-ach-player="${id}"]`);if(ae)ae.innerHTML=aa.length?aa.map(achievementBadgeHtml).join(''):'<span class="selection-achievement-empty">—</span>';
      const contexts=(allStats||[]).filter(x=>x.player_id===id).map(x=>{const tm=x.selection||teams.find(t=>t.id===x.selection_id)||{};const link=tagReferenceLinks.find(l=>l.reference_id===x.selection_id&&l.relation_kind===(x.appearance_status==='called_only'?'status':'membership'));return {...x,selection_code:tm.code||'',selection_name:tm.name||'',tag:link?tagMap.get(link.tag_id):null,player_id:id};}).sort((a,b)=>(teams.findIndex(t=>t.id===a.selection_id))-(teams.findIndex(t=>t.id===b.selection_id)));
      playerContextCache.set(id,contexts);
      const selectionTagIds=new Set(contexts.map(c=>c.tag?.id).filter(Boolean));
      const generic=(et||[]).filter(x=>x.entity_id===id&&!selectionTagIds.has(x.tag_id)).map(x=>tagMap.get(x.tag_id)).filter(Boolean).filter(t=>t.slug!=='general');
      const te=$(`[data-tags-player="${id}"]`);if(te){const buttons=[{...generalStats(contexts),player_id:id,tag:generalTag},...contexts].map(c=>contextButtonHtml(c,c.selection_code===selectedCode)).join('');const genericHtml=generic.map(t=>window.BLEUS3000_TAGS?.chipHtml?window.BLEUS3000_TAGS.chipHtml(t,'selection-linked-tag-chip'):`<span class="selection-main-tag">${esc(t.label_text)}</span>`).join('');te.innerHTML=buttons+genericHtml;$$('[data-context-code]',te).forEach(b=>b.addEventListener('click',()=>applyTileContext(id,b.dataset.contextCode)));}
      tileJerseysByContext(id,allJerseys||[]);
    });
  }
  function tileJerseysByContext(playerId,allJerseys){const tile=$(`[data-player-tile="${playerId}"]`);if(!tile)return;tile._jerseyContexts=(allJerseys||[]).filter(x=>x.player_id===playerId);}
  ...
  async function render(q=''){
    query=q||'';if(loading)return;loading=true;renderLoading();try{if(!await ensureLoaded()){throw new Error('Supabase indisponible');}await hydrateRowTagIds();renderCategoryTabs();ensureSelectionFilters();const all=filterRows(),pages=Math.max(1,Math.ceil(all.length/pageSize));page=Math.min(page,pages);const visible=all.slice((page-1)*pageSize,page*pageSize);const ref=$('#referenceCount');if(ref)ref.textContent=`${all.length} joueur${all.length>1?'s':''}`;const title=$('#referenceModalTitle'),sub=$('#referenceModalSub');if(title)title.textContent='Sélections';if(sub)sub.textContent=`${selectedTeam()?.name||''} · joueurs reliés à toute la base 3615 Bleus`;drawRows(visible,all.length);hydrateContributors(visible).catch(()=>{});}catch(e){console.error(e);const h=$('#referenceEntries');if(h)h.innerHTML='<div class="selection-empty"><strong>Impossible de charger le référentiel.</strong><span>Vérifie la connexion Supabase.</span></div>';}finally{loading=false;}
  }
  ...
  function closeEditor(){const m=$('#selectionPlayerEditorModal');if(m)m.hidden=true;currentEditor=null;pendingPhoto=null;pendingPhotoPreview='';pendingJerseyNumbers=[];pendingAchievements=[];}
  ...
  async function openEditor(playerId=null,contextCode=null){
    if(!canEdit())return alert('Modification réservée aux CONTRIBUTOR, ADMIN et SUPERADMIN.');ensureEditor();if(!teams.length)await loadTeams();
    let contextRows=[];if(playerId){const {data,error}=await client.from('player_selection_stats').select(`id,player_id,selection_id,selections,goals,wins,draws,losses,starts,minutes,international_number,appearance_status,first_year,last_year,data_status,updated_at,selection:selection_teams(id,code,name,gender),player:players!player_selection_stats_player_id_fkey(id,display_name,last_name,birth_date,photo_path,primary_position,secondary_positions,active_source,data_status)`).eq('player_id',playerId);if(error)throw error;contextRows=data||[];}
    const wantedCode=contextCode||(contextRows.some(x=>x.selection?.code===selectedCode)?selectedCode:contextRows[0]?.selection?.code)||selectedCode;const team=teams.find(t=>t.code===wantedCode)||selectedTeam();let row=playerId?contextRows.find(x=>x.selection_id===team.id)||rows.find(x=>x.player_id===playerId):null;let p=publicPlayer(row);if(playerId&&!p?.id){const {data:pp}=await client.from('players').select('id,display_name,last_name,birth_date,photo_path,primary_position,secondary_positions,active_source,data_status').eq('id',playerId).single();p=pp||{};}
    currentEditor={playerId,rowId:row?.id||null,newPlayer:!playerId,selectionId:team.id,selectionCode:team.code,photoPath:p.photo_path||null};pendingPhoto=null;pendingPhotoPreview='';const form=$('#selectionPlayerEditorForm');form.reset();delete form.dataset.removePhoto;if(!form.dataset.appearanceBound){form.appearance_status?.addEventListener('change',()=>syncAppearanceEditor(form));form.dataset.appearanceBound='1';}
    const ctx=$('#selectionEditorContext');if(ctx){ctx.innerHTML=playerId?contextRows.map(x=>{const lk=tagReferenceLinks.find(l=>l.reference_id===x.selection_id&&l.relation_kind===(x.appearance_status==='called_only'?'status':'membership'));const tg=lk?tags.find(t=>t.id===lk.tag_id):null;return `<option value="${esc(x.selection?.code||'')}" ${(x.selection?.code||'')===team.code?'selected':''}>${esc(tg?.label_text||x.selection?.name||x.selection?.code||'Sélection')}</option>`;}).join(''):`<option value="${esc(team.code)}" selected>${esc(team.name)}</option>`;ctx.disabled=!playerId||contextRows.length<2;}
    $('#selectionEditorTitle').textContent=playerId?`Modifier · ${p.display_name}`:'Ajouter un joueur';$('#selectionEditorSub').textContent=team.name;form.display_name.value=p.display_name||'';form.birth_date.value=p.birth_date||'';$('#selectionEditorPositions').innerHTML=positionsEditorHtml(p);for(const k of ['international_number','selections','goals','starts','wins','draws','losses'])form[k].value=row?.[k]??'';form.appearance_status.value=row?.appearance_status||((row?.international_number==null&&Number(row?.selections||0)===0)?'called_only':'capped');syncAppearanceEditor(form);
    const [{data:jersey},{data:et},{data:pa}]=playerId?await Promise.all([client.from('player_jersey_numbers').select('shirt_number').eq('player_id',playerId).eq('selection_id',team.id).order('shirt_number'),client.from('entity_tags').select('tag_id').eq('entity_type','player').eq('entity_id',playerId),client.from('player_achievements').select('achievement_id,achievement_value,selection_id,achievement:achievements(slug,label_text,icon_text,icon_image_path,icon_storage_path,description_short)').eq('player_id',playerId).eq('selection_id',team.id)]):[{data:[]},{data:[]},{data:[]}];pendingJerseyNumbers=[...new Set((jersey||[]).map(x=>Number(x.shirt_number)).filter(Number.isInteger))].sort((a,b)=>a-b);renderEditorJerseyNumbers();const assigned=new Set((et||[]).map(x=>x.tag_id));const membershipDefaults=new Set(tagReferenceLinks.filter(l=>l.reference_id===team.id&&l.relation_kind==='membership').map(l=>l.tag_id));$('#selectionEditorTags').innerHTML=tags.filter(t=>t.slug!=='general').map(t=>`<label class="selection-tag-check"><input type="checkbox" name="tag_ids" value="${t.id}" ${assigned.has(t.id)||(!playerId&&membershipDefaults.has(t.id))?'checked':''}><span>${esc(t.icon_text||'🏷️')} ${esc(t.label_text)}</span></label>`).join('');pendingAchievements=(pa||[]).map(x=>({achievement_id:x.achievement_id,achievement_value:x.achievement_value??null}));renderEditorAchievements();updateEditorPhoto();$('#selectionPlayerEditorModal').hidden=false;
  }
  ...
  async function saveEditor(e){
    e.preventDefault();if(!canEdit()||!client||!currentEditor)return;const form=e.currentTarget,fd=new FormData(form),team=teams.find(t=>t.id===currentEditor.selectionId)||selectedTeam();let playerId=currentEditor.playerId,photoPath=currentEditor.photoPath;
    try{
      if(currentEditor.newPlayer){const name=String(fd.get('display_name')||'').trim();const {data:p,error}=await client.from('players').insert({display_name:name,last_name:name,gender:team.gender,birth_date:fd.get('birth_date')||null,primary_position:fd.getAll('positions').map(String).filter(Boolean)[0]||null,secondary_positions:fd.getAll('positions').map(String).filter(Boolean).slice(1),france_eligibility:true,senior_a_called:team.code==='FRA-A-M',active:true,name_normalized:name.toLocaleLowerCase('fr'),profile_slug:name.toLocaleLowerCase('fr').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')+'-'+Date.now()}).select('id').single();if(error)throw error;playerId=p.id;currentEditor.playerId=playerId;
        ...
        const {error:pe}=await client.from('players').update({display_name:String(fd.get('display_name')||'').trim(),last_name:String(fd.get('display_name')||'').trim(),birth_date:fd.get('birth_date')||null,primary_position:fd.getAll('positions').map(String).filter(Boolean)[0]||null,secondary_positions:fd.getAll('positions').map(String).filter(Boolean).slice(1)}).eq('id',playerId);if(pe)throw pe;const appearanceStatus=fd.get('appearance_status')==='called_only'?'called_only':'capped';const statPayload={appearance_status:appearanceStatus};for(const k of ['international_number','selections','goals','starts','wins','draws','losses'])statPayload[k]=fd.get(k)===''?null:Number(fd.get(k));if(appearanceStatus==='called_only'){statPayload.international_number=null;for(const k of ['selections','goals','starts','wins','draws','losses'])statPayload[k]=0;}const {error:se}=await client.from('player_selection_stats').update(statPayload).eq('player_id',playerId).eq('selection_id',team.id);if(se)throw se;
      }
      ...
      {const selectedAchievements=pendingAchievements.map(a=>({achievement_id:a.achievement_id,achievement_value:a.achievement_value===null||a.achievement_value===undefined||a.achievement_value===''?null:Math.max(0,Number(a.achievement_value)||0)}));const {error:ade}=await client.from('player_achievements').delete().eq('player_id',playerId).eq('selection_id',team.id);if(ade)throw new Error('Accomplissements · suppression : '+ade.message);if(selectedAchievements.length){const payload=selectedAchievements.map(a=>({player_id:playerId,selection_id:team.id,achievement_id:a.achievement_id,achievement_value:a.achievement_value,added_by:window.C3K_ACCOUNT_STATE?.profile?.id||null}));const {error:ae}=await client.from('player_achievements').insert(payload);if(ae)throw new Error('Accomplissements · ajout : '+ae.message);}}
      ...
      playerContextCache.delete(playerId);await client.rpc('record_selection_contribution',{p_player_id:playerId,p_type:currentEditor.newPlayer?'create':'edit'});closeEditor();rows=[];await loadRows();render(query);window.BLEUS3000_PLAYERS_DB?.reload?.();window.BLEUS3000_STATISTICS?.refresh?.();
