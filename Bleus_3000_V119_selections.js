...
(() => {
  ...
  function achievementEditorHtml(assigned=[]){const map=new Map((assigned||[]).map(x=>[x.achievement_id,x]));if(!achievements.length)return '<span class="selection-achievement-empty">Aucun accomplissement disponible.</span>';return achievements.map(a=>{const cur=map.get(a.id),checked=!!cur,value=cur?.achievement_value??'';return `<div class="selection-achievement-editor-row" data-achievement-id="${esc(a.id)}"><span class="selection-achievement-editor-icon">${achievementBadgeHtml({achievement:a})}</span><span class="selection-achievement-editor-name"><input type="checkbox" data-achievement-check ${checked?'checked':''}> <strong>${esc(a.label_text)}</strong><small>${esc(a.description_short||'')}</small></span><label class="selection-achievement-value-label">Nombre<input type="number" min="0" step="1" data-achievement-value value="${esc(value)}" placeholder="ex. 36" ${checked?'':'disabled'}></label></div>`;}).join('');}
  ...
  function ensureEditor(){
    ...
    $$('[data-selection-editor-close]').forEach(b=>b.addEventListener('click',closeEditor));$('#selectionPlayerEditorForm').addEventListener('submit',saveEditor);$('#selectionEditorPhotoInput').addEventListener('change',preparePhoto);$('#selectionEditorPhotoRemove').addEventListener('click',()=>{pendingPhoto=null;pendingPhotoPreview='';$('#selectionPlayerEditorForm').dataset.removePhoto='1';updateEditorPhoto();});$('#selectionEditorJerseyAdd').addEventListener('click',addEditorJerseyNumber);$('#selectionEditorJerseyList').addEventListener('click',e=>{const b=e.target.closest('[data-remove-jersey]');if(!b)return;pendingJerseyNumbers=pendingJerseyNumbers.filter(n=>n!==Number(b.dataset.removeJersey));renderEditorJerseyNumbers();});$('#selectionEditorAchievements').addEventListener('change',e=>{const row=e.target.closest('.selection-achievement-editor-row');if(!row)return;const check=row.querySelector('[data-achievement-check]'),value=row.querySelector('[data-achievement-value]');if(check&&value)value.disabled=!check.checked;});
  }
  ...
  async function saveEditor(e){
    ...
    try{
      ...
      {const selectedAchievements=$$('.selection-achievement-editor-row',form).filter(r=>r.querySelector('[data-achievement-check]')?.checked).map(r=>({achievement_id:r.dataset.achievementId,achievement_value:(()=>{const raw=r.querySelector('[data-achievement-value]')?.value;return raw===''||raw==null?null:Math.max(0,Number(raw)||0);})()}));const {error:ade}=await client.from('player_achievements').delete().eq('player_id',playerId).eq('selection_id',team.id);if(ade)throw new Error('Accomplissements · suppression : '+ade.message);if(selectedAchievements.length){const payload=selectedAchievements.map(a=>({player_id:playerId,selection_id:team.id,achievement_id:a.achievement_id,achievement_value:a.achievement_value,added_by:window.C3K_ACCOUNT_STATE?.profile?.id||null}));const {error:ae}=await client.from('player_achievements').insert(payload);if(ae)throw new Error('Accomplissements · ajout : '+ae.message);}}
