/* 3615 Bleus V1.1.61.14 — Sélections · filtres de poste relationnels */
(() => {
  'use strict';
  ...
  let sortKey='selections',sortDir='desc',jerseyFilter='',tagFilter='',positionFilter='';
  ...
  const JERSEY_DIGIT_ASSET=n=>window.BLEUS3000_FLOCKING?.assetUrl?.(window.BLEUS3000_FLOCKING?.getStyle?.()||'france-2024',n)||`flocages_assets/france-2024/${n}.png`;
  ...
  function renderCategoryTabs(){
    ...
    $$('[data-selection-code]',host).forEach(b=>b.addEventListener('click',async()=>{selectedCode=b.dataset.selectionCode;page=1;rows=[];jerseyFilter='';tagFilter='';positionFilter='';renderLoading();await loadRows();render(query);}));
  }
  ...
  function ensureSelectionFilters(){
    ...
    $('#selectionFilterReset',bar)?.addEventListener('click',()=>{sortKey='selections';sortDir='desc';jerseyFilter='';tagFilter='';positionFilter='';page=1;render(query);});
  }
  ...
  function drawRows(visible,total){
    ...
    host.innerHTML=`<div class="selection-grid">${visible.map(r=>{const p=publicPlayer(r);return `<article class="selection-player-tile" data-player-tile="${esc(r.player_id)}">
      <div class="selection-player-head"><div class="selection-photo-frame" style="${borderStyle()}"><div class="selection-photo-inner">${playerPhoto(p)}</div></div><div class="selection-player-title">${appearanceBadge(r)}<h3>${esc(p.display_name||'Joueur')}</h3><div class="selection-dob">Né(e) le ${esc(fmtDate(p.birth_date))}</div><div class="selection-position ${playerPositions(p).length?'':'is-empty'}"><span class="selection-position-label">Poste${playerPositions(p).length>1?'s':''} ·</span><span class="selection-position-inline" data-position-player="${esc(r.player_id)}">${esc(positionsLabel(p)||'Non renseigné')}</span></div><div class="selection-tile-actions">${canEdit()?`<button type="button" data-edit-player="${r.player_id}" title="Modifier">✎</button>`:''}</div></div></div>
  ...
  async function selectCategory(code){selectedCode=code||'FRA-A-M';rows=[];page=1;jerseyFilter='';tagFilter='';positionFilter='';await render(query);}
  ...
  async function saveEditor(e){
    ...
    try{
      ...
      const savedContext=currentEditor.selectionCode;const contributionType=currentEditor.newPlayer?'create':'edit';playerContextCache.delete(playerId);await client.rpc('record_selection_contribution',{p_player_id:playerId,p_type:contributionType});rows=[];await loadRows();render(query);window.BLEUS3000_PLAYERS_DB?.reload?.();window.BLEUS3000_STATISTICS?.invalidate?.();await openEditor(playerId,savedContext);const saved=$('#selectionEditorSaveStatus');if(saved){saved.textContent='Enregistré ✓ · la fenêtre reste ouverte';setTimeout(()=>{if(saved.isConnected)saved.textContent='';},1800);}
