...
(() => {
  ...
  async function saveEditor(e){
    ...
    try{
      ...
      const savedContext=currentEditor.selectionCode;const contributionType=currentEditor.newPlayer?'create':'edit';playerContextCache.delete(playerId);await client.rpc('record_selection_contribution',{p_player_id:playerId,p_type:contributionType});rows=[];await loadRows();render(query);window.BLEUS3000_PLAYERS_DB?.reload?.();window.BLEUS3000_STATISTICS?.refresh?.();await openEditor(playerId,savedContext);const saved=$('#selectionEditorSaveStatus');if(saved){saved.textContent='Enregistré ✓ · la fenêtre reste ouverte';setTimeout(()=>{if(saved.isConnected)saved.textContent='';},1800);}
