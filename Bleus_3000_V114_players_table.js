...
(() => {
  ...
  let search='', positionFilter='', teamFilter='';
  ...
  const norm=v=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase('fr');
  ...
  function buildFilters(){
    ...
    pos.value=positionFilter;team.value=teamFilter;
  }
  ...
  function bind(){
    ...
    $('#playersDbReset')?.addEventListener('click',()=>{search='';positionFilter='';teamFilter='';page=1;sortKey='name';sortDir='asc';if($('#playersDbSearch'))$('#playersDbSearch').value='';buildFilters();$$('[data-player-db-sort]').forEach(b=>b.classList.remove('is-active'));apply();});
  ...
  window.BLEUS3000_PLAYERS_DB={reload:load,get players(){return players;}};
