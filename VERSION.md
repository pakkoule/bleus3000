...
## V1.1.59
- Suppression du référentiel **Bibliographie & Médias**.
- Renommage **Équipements → Maillots**.
- Nouveau rendu de tuile Maillot centré sur la photo principale et la galerie.
- Tags/relations **Équipes affiliées** puis séparateur visuel puis **Compétitions affiliées**.
- Éditeur Maillot : usage, saison, équipementier, référence, template, couleurs, col, manches, étoiles, flocage, version, matière, technologie, dates d'utilisation, notes et sources.
- Nouveau schéma Supabase relationnel : `jerseys`, `jersey_selection_teams`, `jersey_competitions`, `jersey_photos` + bucket public `jersey-photos`.
- Upload multi-photo PNG/JPG/WebP jusqu'à 10 Mo par image ; première photo utilisable comme photo principale.
- Galerie photo intégrée : clic sur la grande photo d’une tuile pour parcourir face, dos, détails et photos portées.

## V1.1.58
- Intégration de l’outil **Archives FFF** dans la barre d’outils avec icône monochrome type archive ZIP.
- Ajout d’un extracteur médias FFF / media.fff.fr avec filtres, export CSV/JSON, mode démo et support du **match de référence** issu des tuiles Match.
- Ajout de la Netlify Function `fff-media-extractor` pour les recherches publiques côté serveur.

# 3615 Bleus — V1.1.57

## Nouveautés V1.1.57 · Statistiques V2

Le bloc **Statistiques** est entièrement enrichi autour de la base relationnelle existante.

### 🔵 Parcours Bleu
- diagramme U17 → U18 → U19 → U20 → Espoirs → A ;
- volumes par catégorie ;
- taux de conservation entre deux catégories ;
- taux U17 → A ;
- clic sur un niveau ou une transition pour afficher les joueurs concernés.

### 👥 Générations
- sélection d'une année de naissance ;
- parcours de chaque joueur/joueuse à travers les catégories ;
- niveau maximal atteint ;
- taux de conservation de la génération entre chaque catégorie ;
- accès direct à la tuile joueur.

### 🤝 Connexions
- coéquipiers les plus fréquents à partir des feuilles de match disponibles ;
- détail des matchs partagés ;
- module buteur + passeur prêt à s'alimenter automatiquement avec les passes décisives.
...
- compétition ;
- adversaire ;
- MJ / V / N / D / BP / BC / % de victoires ;
- chaque ligne ouvre les matchs exacts utilisés dans le calcul.

### 🏅 Records
- maintien des classements buteurs, sélections et capitanat dans la V2.

## Données et sécurité

- aucune migration Supabase V1.1.57 n'est nécessaire ;
- aucun total n'est écrit dans la base : les statistiques sont calculées à la volée depuis les relations existantes ;
- les limites de couverture des feuilles de match, dates de naissance et passes décisives sont affichées dans l'interface.

La V1.1.56 U18 Féminines et toutes les versions précédentes restent incluses dans le package complet.
